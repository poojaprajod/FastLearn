   ...
   final String PASS = "toor";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (student_id.length()!=5){
            errors.add("INCORRECT Student ID");
        }
     ...
     try{
      ...
      String sql = "insert into student(student_id,first_name,last_name,course_id,faculty_id,centre_id,phone,address) values (?,?,?,?,?,?,?,?)"; 
      PreparedStatement pst =  conn.prepareStatement(sql);
      pst.setString(1,student_id);
      pst.setString(2,fname);
      pst.setString(3,lname);
      pst.setString(4,course_id);
      pst.setString(5,faculty_id);
      pst.setString(6,centre_id);
      pst.setLong(7, phone);
     ...
     if (!errors.isEmpty()){
         ...
         RequestDispatcher v=request.getRequestDispatcher("add_student.jsp");
         v.forward(request, response);
         return;
     }        
RequestDispatcher v=request.getRequestDispatcher("success.jsp?page=add_student");
         v.forward(request, response);
    }
